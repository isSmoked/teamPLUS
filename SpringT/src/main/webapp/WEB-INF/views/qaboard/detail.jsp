...
<title>${qaboardVO.qaboard_title }</title>
<style type="text/css">
...
<body>
	<h6><a href="list" style="color: black; 
	          text-decoration: none;"><strong>질문게시판</strong></a> > ${qaboardVO.qaboard_bno }</h6>
	<hr>
	<!-- 본문 -->
	<H2><c:out value="${qaboardVO.qaboard_title }"/></H2>
	<fmt:formatDate value="${qaboardVO.qaboard_date }" pattern="yyyy-MM-dd HH:mm:ss" var="qaboard_date" />
		작성자 : ${qaboardVO.userid } | 작성일 : ${qaboard_date } | 	
		<!-- 공지글 설정 (관리자 전용) -->
      	<c:if test="${part eq 'manager' }"> 
       	  <select id="notice">
		  	<option value="0" <c:if test="${qaboardVO.qaboard_notice eq '0' }">selected</c:if>>공지_0</option>
		  	<option value="1" <c:if test="${qaboardVO.qaboard_notice eq '1' }">selected</c:if>>공지_1</option>
		  	<option value="2" <c:if test="${qaboardVO.qaboard_notice eq '2' }">selected</c:if>>공지_2</option>
		  	<option value="3" <c:if test="${qaboardVO.qaboard_notice eq '3' }">selected</c:if>>공지_3</option>
		  </select>
	...
	<!-- 하단 버튼 -->
	<div>
		<c:if test="${loginId ne null }"><input type="button" id="bookmark" class="btn btn-light" style="background: silver;" onclick="bookmark()" value="찜"></c:if>&nbsp;&nbsp;&nbsp;
		
		<a href="list?page=${page }&searchType=${searchType }&keyword=${keyword }"><input type="button" class="btn btn-light" value="글 목록"></a>
		<c:if test="${qaboardVO.userid eq loginId and blacklist ne 1}">
			<a href="update?bno=${qaboardVO.qaboard_bno }&page=${page}"><input type="button" class="btn btn-light" value="글 수정"></a>
			<a href="delete?bno=${qaboardVO.qaboard_bno }" class="deleteLink" onclick="return false"><input type="button" class="btn btn-light" onclick="deleteConfirm()" value="글 삭제"></a>
		</c:if>
	</div>
	<br>
	...
	<input type="hidden" id="bno" value="${qaboardVO.qaboard_bno }">
		...
		// ======================================================================================
			...
			function bookmarkCheck(){
				...
				var bno = $('#bno').val();
 				var bookmark = {
					userid : id,
					bookmark_table_name : "qaboard_table",
					bookmark_bno : bno
				};
			...
			var sock = new WebSocket('ws://' + window.location.host + '/prj/qaboard/websocket');
	  	
      		var message2 = ""; // 보낼 메시지 가공!!
